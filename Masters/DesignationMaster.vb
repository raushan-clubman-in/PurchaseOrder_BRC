...
Public Class DesignationMaster
    ...
    Private Sub DesignationMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub checkValidation()
        ...
        If Trim(txt_dept.Text) = "" Then
            MessageBox.Show("DESINATION Code can't be blank ", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_dept.Focus()
            Exit Sub
        End If
        If Trim(txt_deptname.Text) = "" Then
            MessageBox.Show("DESINATION Name can't be blank ", MyCompanyName, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txt_deptname.Focus()
            Exit Sub
        End If
        boolchk = True
    ...
    Private Sub cmd_add_Click(sender As Object, e As EventArgs) Handles cmd_add.Click
        ...
        If cmd_add.Text = "Save [F7]" Then
            ...
            strsql = "insert into PO_DESIGNATION (code,name,freeze,adduser,adddatetime)"
            ...
            gconnection.dataOperation(1, strsql, "PO_DESIGNATION")
        ...
        Else
            ...
            gconnection.dataOperation(2, strsql, "PO_DESIGNATION")
            Me.cmd_clear_Click(sender, e)
    ...
    Private Sub cmd_help_Click(sender As Object, e As EventArgs) Handles cmd_help.Click
        Try
            gSQLString = "SELECT ISNULL(code,'') AS code,ISNULL(name,'') AS               name FROM PO_DESIGNATION"
            M_WhereCondition = " "
            Dim vform As New ListOperattion1_N
            vform.Field = "code,name"
            vform.vFormatstring = "    CODE              |                                                      name                                                             "
            vform.vCaption = "DESIGNATION MASTER HELP"
            vform.KeyPos = 0
            vform.KeyPos1 = 1
            vform.ShowDialog(Me)
    ...
    Private Sub txt_dept_Validated(sender As Object, e As EventArgs) Handles txt_dept.Validated
        Dim strsql As String
        If Trim(txt_dept.Text) <> "" Then
            strsql = "SELECT * FROM PO_DESIGNATION WHERE CODE='" & Trim(txt_dept.Text) & "'"
            gconnection.getDataSet(strsql, "PO_DESIGNATION")
            If gdataset.Tables("PO_DESIGNATION").Rows.Count > 0 Then
                txt_dept.Text = Trim(gdataset.Tables("PO_DESIGNATION").Rows(0).Item("CODE"))
                txt_deptname.Text = Trim(gdataset.Tables("PO_DESIGNATION").Rows(0).Item("NAME"))
                txt_deptname.Focus()

                If gdataset.Tables("PO_DESIGNATION").Rows(0).Item("Freeze") = "Y" Then
                    Me.lbl_Freeze.Visible = True
                    Me.lbl_Freeze.Text = ""
                    Me.lbl_Freeze.Text = "Record Freezed  On " & Format(CDate(gdataset.Tables("PO_DESIGNATION").Rows(0).Item("AddDatetime")), "dd-MMM-yyyy")
